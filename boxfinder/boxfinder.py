...
def red_left (pix, x, y):
    for i in range(x-1):
        (r, g, b) = pix[x, y]
        print("XY:", x,y,"HSV:", rgb_to_hsv(r,g,b), "color:",color_recognizor(r,g,b))
        if (color_recognizor(r,g,b) == "Red"):
            return x, y
        x -= 1    
...
def find_box(pix, x, y, width, height, image):
    ...
    for i in range(bottom_red_y - up_red_y):
        for k in range(right_red_x - left_red_x):
            ...
            if (color_recognizor(r,g,b) == "Black" and is_point_within_circle(x, y, ((right_red_x - left_red_x)+(up_red_y - bottom_red_y)/2), start_x, start_y)):
...
def remove_last_digit(pix, top_y, bottom_y, right_x, left_x):
    ...
    for i in range (search_range):
        ...
        if (color_recognizor(r,g,b) == "Black"):
            black_pixel = True
        else:
            black_pixel = False
...
def return_coordinates(image, center_coordinate):
    ...
    perfect_image(pix, top_left_x, top_left_y, bottom_right_x, bottom_right_y)
...
def perfect_image(pix, top_left_x, top_left_y, bottom_right_y, bottom_right_x):
    ...
    for i in range (bottom_right_y - top_left_y):
        for k in range (bottom_right_x - top_left_x):
            (r,g,b) = pix[start_x, start_y]
            if (color_recognizor(r,g,b) == "Black"):
                pix[start_x, start_y] = (0, 0, 0)
            else:
                pix[start_x, start_y] = (255, 255, 255)
            start_x += 1
        start_x = x_origin
        start_y += 1
